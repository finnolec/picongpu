        ...
        namespace incidentField
        {
            namespace profiles
            {
                namespace detail
                {
                     ...
                     */
                    template<typename T_Params>
                    struct SlopeTiltParam<T_Params, HasTilt<T_Params>>
                    {
                        // unit: radian
                     ...
                     * The branching in terms of if and how user sets a tilt is encapculated in SlopeTiltParam.
                     *
                     * @tparam T_Params user (SI) parameters
                     */
                    template<typename T_Params>
                    struct GaussianBeamSlopesUnitless
                        : public BaseParamUnitless<T_Params>
                        , public SlopeTiltParam<T_Params>
                    {
                        //! User SI parameters
                     ...
                     */
                    template<typename T_Params>
                    struct GaussianBeamSlopesFunctorIncidentE
                        : public GaussianBeamSlopesUnitless<T_Params>
                        , public incidentField::detail::BaseFunctorE<T_Params>
                    {
                        //! Unitless parameters type
                        using Unitless = GaussianBeamSlopesUnitless<T_Params>;
                         ...
                         */
                        HINLINE GaussianBeamSlopesFunctorIncidentE(float_X const currentStep, float3_64 const unitField)
                    ...
                    private:
                        ...
                        HDINLINE float_X getValue(floatD_X const& totalCellIdx, float_X const phaseShift) const
                        {
                            ...
                            float_X const polpos = std::abs(pos[1]);
                            if((polpos >= Unitless::POL_PLAT_MAX) && (polpos <= (Unitless::POL_PLAT_MAX + Unitless::POL_RAMP_LENGTH))){
                                transversalFactor *= (1.0 + math::cos(PI * (polpos - Unitless::POL_PLAT_MAX) / Unitless::POL_RAMP_LENGTH)) / 2.0;
                            } else if(polpos > (Unitless::POL_PLAT_MAX + Unitless::POL_RAMP_LENGTH)) {
                                transversalFactor *= 0.0;
                            }
                            
                            float_X const offpos = std::abs(pos[2]);
                            if((offpos >= Unitless::OFF_PLAT_MAX) && (offpos <= (Unitless::OFF_PLAT_MAX + Unitless::OFF_RAMP_LENGTH))){
                                transversalFactor *= (1.0 + math::cos(PI * (offpos - Unitless::OFF_PLAT_MAX) / Unitless::OFF_RAMP_LENGTH)) / 2.0;
                            } else if(offpos > (Unitless::OFF_PLAT_MAX + Unitless::OFF_RAMP_LENGTH)) {
                                transversalFactor *= 0.0;
                            }
...
/*
                            if((pos[1] <= (Unitless::POL_PLAT_MIN - Unitless::POL_RAMP_LENGTH)) || (pos[1] >= (Unitless::POL_PLAT_MAX + Unitless::POL_RAMP_LENGTH))){
                                ...
                                transversalFactor *= (1.0 - math::cos(
                                    PI * std::abs(pos[1] - Unitless::POL_PLAT_MIN + Unitless::POL_RAMP_LENGTH) / Unitless::POL_RAMP_LENGTH)) / 2.0;
                            } else if ((pos[1] > Unitless::POL_PLAT_MAX) && (pos[1] < (Unitless::POL_PLAT_MAX + Unitless::POL_RAMP_LENGTH))) {
                                transversalFactor *= (1.0 + math::cos(PI * std::abs(pos[1] - Unitless::POL_PLAT_MAX) / Unitless::POL_RAMP_LENGTH)) / 2.0;
                            }
*/
/*
                            if((pos[2] <= (Unitless::OFF_PLAT_MIN - Unitless::OFF_RAMP_LENGTH)) || (pos[2] >= (Unitless::OFF_PLAT_MAX + Unitless::OFF_RAMP_LENGTH))){
                                ...
                                transversalFactor *= (1.0 - math::cos(
                                    PI * std::abs(pos[2] - Unitless::OFF_PLAT_MIN + Unitless::OFF_RAMP_LENGTH) / Unitless::OFF_RAMP_LENGTH)) / 2.0;
                            } else if ((pos[2] > Unitless::OFF_PLAT_MAX) && (pos[2] < (Unitless::OFF_PLAT_MAX + Unitless::OFF_RAMP_LENGTH))) {
                                transversalFactor *= (1.0 + math::cos(PI * std::abs(pos[2] - Unitless::OFF_PLAT_MAX) / Unitless::OFF_RAMP_LENGTH)) / 2.0;
                            }
*/
                            return transversalFactor * envelope * etrans / etrans_norm;
                        }
                ...
                struct GaussianBeamSlopes
                {
                    ...
                    static HINLINE std::string getName()
                    {
                        ...
                        using SlopeTiltParam = detail::SlopeTiltParam<T_Params>;
                        bool isTilted = (std::abs(SlopeTiltParam::TILT_AXIS_1) + std::abs(SlopeTiltParam::TILT_AXIS_2) > 0);
                        return isTilted ? "PulseFrontTilt" : "GaussianBeamSlopes";
                    }
                };
            ...
            namespace detail
            {
                 ...
                 */
                template<typename T_Params>
                struct GetFunctorIncidentE<profiles::GaussianBeamSlopes<T_Params>>
                {
                    using type = profiles::detail::GaussianBeamSlopesFunctorIncidentE<T_Params>;
                };
                 ...
                 */
                template<typename T_Params>
                struct GetFunctorIncidentB<profiles::GaussianBeamSlopes<T_Params>>
                {
                    using type = detail::ApproximateIncidentB<
                        typename GetFunctorIncidentE<profiles::GaussianBeamSlopes<T_Params>>::type>;
                };
            } // namespace detail
        } // namespace incidentField
    } // namespace fields
