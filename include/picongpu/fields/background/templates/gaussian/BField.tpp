...
namespace picongpu
{
    ...
    namespace templates
    {
        ...
        namespace gaussian
        {
            ...
            HDINLINE float3_X BField::getBfield_Normalized_Linear_X(
                ...
                const float_64 time) const
            {
                 ...
                 */
                return float3_X(float_X(calcBx_Linear_X(pos[0], time)), float_X(By_rot), float_X(Bz_rot));
            }

            HDINLINE float3_X BField::getBfield_Normalized_Linear_YZ(
                const pmacc::math::Vector<floatD_64, detail::numComponents>& bFieldPositions_SI,
                const float_64 time) const
            {
                typedef pmacc::math::Vector<float3_64, detail::numComponents> PosVecVec;
                ...
                /* Finally, the B-field normalized to the peak amplitude. */
                return float3_X(float_X(calcBx_Linear_YZ(pos[0], time)), float_X(By_rot), float_X(Bz_rot));
            }
            ...
            HDINLINE BField::float_T BField::calcBx_Linear_X(const float3_64& pos, const float_64 time) const
            {
                ...
                const complex_T Ex = w0 / wz
                    * math::exp(complex_T(0, 1) * (omega0 * t - k0 * z) + complex_T(0, 1) * phiz
                                + complex_T(0, 1) * phi0
                                - (x * x + y * y)
                                    * (float_T(1.0) / (wz * wz) + complex_T(0, 1) * k0 * RzInv / float_T(2.0)));
                const complex_T Ex_0
                    = Ex * math::exp(-(t - z / cspeed) * (t - z / cspeed) / (float_T(2.0) * tauG * tauG));
                const complex_T Bx_1 = s * s * xi * eta * (-float_T(4.0) * theta * theta) * Ex_0
                    * (float_T(1.0)
                       + sigma * epsilon * theta
                           * (-float_T(2.0) * zeta * complex_T(0, 1) + float_T(2.0) * rho2
                              - float_T(2.0) * rho2 * theta * zeta - float_T(2.0)));
             ...
             * \param time Absolute time (SI, including all offsets and transformations)
             *             for calculating the field */
            HDINLINE BField::float_T BField::calcBy_Linear_X(const float3_64& pos, const float_64 time) const
            {
                using complex_T = pmacc::math::Complex<float_T>;
                using complex_64 = pmacc::math::Complex<float_64>;
                /* Unit of speed */
                ...
                const complex_T Ex_0
                    = Ex * math::exp(-(t - z / cspeed) * (t - z / cspeed) / (float_T(2.0) * tauG * tauG));
                const complex_T By_1 = Ex_0
                    * (float_T(1.0)
                       + sigma * epsilon * theta
                           * (-float_T(2.0) * zeta * complex_T(0, 1) + float_T(2.0) * rho2
                              - float_T(2.0) * rho2 * theta * zeta)
                       + s * s * (-float_T(2.0) * rho2 * theta * theta + float_T(4.0) * theta * theta * xi * xi)
                           * (float_T(1.0)
                              + sigma * epsilon * theta
                                  * (-float_T(2.0) * zeta * complex_T(0, 1) + float_T(2.0) * rho2
                                     - float_T(2.0) * rho2 * theta * zeta - float_T(2.0))));
             ...
             * \param time Absolute time (SI, including all offsets and transformations)
             *             for calculating the field */
            HDINLINE BField::float_T BField::calcBz_Linear_X(const float3_64& pos, const float_64 time) const
            {
                using complex_T = pmacc::math::Complex<float_T>;
                using complex_64 = pmacc::math::Complex<float_64>;
                /* Unit of speed */
             ...
             * \param time Absolute time (SI, including all offsets and transformations)
             *             for calculating the field */
            HDINLINE BField::float_T BField::calcBx_Linear_YZ(const float3_64& pos, const float_64 time) const
            {
             ...
             * \param time Absolute time (SI, including all offsets and transformations)
             *             for calculating the field */
            HDINLINE BField::float_T BField::calcBy_Linear_YZ(const float3_64& pos, const float_64 time) const
            {
             ...
             * \param time Absolute time (SI, including all offsets and transformations)
             *             for calculating the field */
            HDINLINE BField::float_T BField::calcBz_Linear_YZ(const float3_64& pos, const float_64 time) const
            {
